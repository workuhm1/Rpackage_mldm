#' Biplot for fitted MLD model.
#' 
#' @param object An object of fitted model.
#' @param myX Range for X-axis.
#' @param myY Range for Y-axis.
#' @param resp.var.labels Labels of the reponse variable.
#' 
#' 

biplot.mldm <- function(object, myX = c(-4,4), myY = c(-4,4), 
                        resp.var.labels = NULL, 
                        ...){
  ...
  # Get significant explanatory variables
  XX = list(M)
  smv = list(2)
  vnames = list(2)
  for(m in 2:(M+1))
  ...
  # Define variable axis
  axcol = 'red'
  lbcol = 'blue'
  lbcex = 1.2
  axcex = 0.9
  
  # first the BB are defined
